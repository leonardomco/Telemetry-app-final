...
def donnees(annee: int, circuit: str, session: str, pilote: str):
    # 1. Early validation check
    if annee < 2018 or annee > 2025:
        raise ValueError(f"Erreur: année {annee} non acceptée. Doit être entre 2018 et 2025.")

    # 2. Fetch session and load telemetry safely
    epreuve = fastf1.get_session(annee, circuit, session)
    try:
        epreuve.load(telemetry=True, laps=True, weather=True)
    except Exception as e:
        raise RuntimeError(f"FastF1 network failure when loading session: {e}")
    ...
    # --- Environmental & Aero Calculations ---
